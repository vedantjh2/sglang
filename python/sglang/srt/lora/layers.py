...
# These are opaque custom ops (NOT split ops) so they are captured INSIDE
# CUDA graphs. The lora_backend is retrieved from ForwardContext instead of
# being accessed as a Python attribute, which dynamo cannot trace.
...
def _lora_shrink_and_expand_fake(
    x: torch.Tensor,
    A_buffer: torch.Tensor,
    B_buffer: torch.Tensor,
    output_offset: torch.Tensor,
...
direct_register_custom_op(
    op_name="lora_shrink_and_expand",
    op_func=_lora_shrink_and_expand,
    mutates_args=["base_output"],
    fake_impl=_lora_shrink_and_expand_fake,
)
...
direct_register_custom_op(
    op_name="lora_gate_up",
    op_func=_lora_gate_up,
    mutates_args=["base_output"],
    fake_impl=_lora_gate_up_fake,
)
...
class ColumnParallelLinearWithLoRA(BaseLayerWithLoRA):
    ...
    def apply_lora(self, base_output: torch.Tensor, x: torch.Tensor) -> torch.Tensor:
        if get_forward_context() is not None:
            torch.ops.sglang.lora_shrink_and_expand(
